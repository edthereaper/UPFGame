...
void CalAnimation::setUserData(Cal::UserData userData)
{
  m_userData = userData;
}
...
Cal::UserData CalAnimation::getUserData() const
{
  return m_userData;
}
...
void CalAnimation::checkCallbacks(float animationTime,CalModel *model)
{
  ...
  for (size_t i=0; i<list.size(); i++)
  {
    ...
    list[i].callback->AnimationUpdate(animationTime, model, model->getUserData(), getUserData());
...
void CalAnimation::completeCallbacks(CalModel *model)
{
  std::vector<CalCoreAnimation::CallbackRecord>& list = m_pCoreAnimation->getCallbackList();
  for (size_t i=0; i<list.size(); i++)
    list[i].callback->AnimationComplete(model, model->getUserData(), getUserData());
}
