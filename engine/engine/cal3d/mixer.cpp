...
namespace utils{
	extern bool dbg(const char* fmt, ...);
}
...
/** Executes an animation action.
  ...
  *         \li \b false if an error happend
  *****************************************************************************/
bool CalMixer::executeAction(int id, float delayIn, float delayOut, float weightTarget, bool autoLock, bool loop)
{

  logic_prev_translation_is_valid = false;
  ...
  pAnimationAction->execute(delayIn, delayOut, weightTarget, autoLock, loop);
...
/** Clears an active animation action.
  ...
  *****************************************************************************/
bool CalMixer::removeAction(int id, float timeout)
{
  // get the core animation
  CalCoreAnimation *pCoreAnimation;
  pCoreAnimation = m_pModel->getCoreModel()->getCoreAnimation(id);
  ...
  while(iteratorAnimationAction != m_listAnimationAction.end())
  {
    ...
    if((*iteratorAnimationAction)->getCoreAnimation() == pCoreAnimation )
    {
      ...
      if ((*iteratorAnimationAction)->remove(timeout)) {
        // found, so remove
...
void CalMixer::updateSkeleton()
{
	...
	// Fixme!
	logic_delta_bone_id = m_pModel->getCoreModel()->getBoneId("Bip001");
	logic_delta_dummy_bone_id = m_pModel->getCoreModel()->getBoneId("Dummy");
	...
	for (iteratorAnimationAction = m_listAnimationAction.begin(); iteratorAnimationAction != m_listAnimationAction.end(); ++iteratorAnimationAction)
	{
		...
		for (iteratorCoreTrack = listCoreTrack.begin(); iteratorCoreTrack != listCoreTrack.end(); ++iteratorCoreTrack)
		{
			...
			if (DummyTime){
				if (bone_id == logic_delta_bone_id) {
					translation = root_pivot;
				}
				if (bone_id == logic_delta_dummy_bone_id) {
					if (!isCycle){
						if (!logic_prev_dummy_translation_is_valid) {
							CalQuaternion logic_prev__dummy_rotation;
							(*iteratorCoreTrack)->getState(0.f, logic_prev_dummy_translation, logic_prev__dummy_rotation);
							logic_prev_dummy_translation_is_valid = true;
						}
						logic_delta_dummy_translation = translation - logic_prev_dummy_translation;
						logic_prev_dummy_translation = translation;
					}
				}
			}
	...
	logic_delta_dummy_translation *= root_rotation;		 //DAVID
	...
	{
		// Get old trans
		CalVector new_trans = pSkeleton->getVectorBone()[*iteratorRootBoneId]->getTranslation();
		new_trans -= root_pivot;
		new_trans *= root_rotation;
		new_trans += root_translation;
		new_trans += root_pivot;
		CalQuaternion new_rot = pSkeleton->getVectorBone()[*iteratorRootBoneId]->getRotation();
		new_rot *= root_rotation;
		// Set new trans
		pSkeleton->getVectorBone()[*iteratorRootBoneId]->setTranslation(new_trans);
		pSkeleton->getVectorBone()[*iteratorRootBoneId]->setRotation(new_rot);
	}
