...
public:
  ...
  bool executeAction(int id, float delayIn, float delayOut, float weightTarget = 1.0f,
      bool autoLock=false, bool loop=false);
  bool removeAction(int id, float timeout);
...
protected:
  ...
  // MCV ---------------------
  CalVector      root_translation;
  CalQuaternion  root_rotation;
  CalVector      root_pivot;

  bool DummyTime;
  bool isCycle;

  int            logic_delta_bone_id;     // ...
  CalVector      logic_delta_translation;
  CalVector      logic_prev_translation;
  bool           logic_prev_translation_is_valid;

  bool           logic_prev_dummy_translation_is_valid;
  CalVector      logic_prev_dummy_translation;
  int            logic_delta_dummy_bone_id;
  CalVector      logic_delta_dummy_translation;

  CalQuaternion      logic_delta_rotation;

public:

	void setRootTranslation(CalVector new_root) {
		root_translation = new_root;
	}
	void setRootRotation(CalQuaternion new_root) {
		root_rotation = new_root;
	}
	void setRootPivot(CalVector pivot) {
		root_pivot = pivot;
	}
	void setDummyTime(bool b)	{ DummyTime = b; }
	void setisCycle(bool b)	{ isCycle = b; }

	CalVector getAndClearDummyLogicTranslation() {
		CalVector d = logic_delta_dummy_translation;
		logic_delta_dummy_translation.clear();
		return d;
	}

	CalVector getAndClearLogicTranslation() {
		CalVector d = logic_delta_translation;
		logic_delta_translation.clear();
		return d;
	}

	CalQuaternion getAndClearLogicRotation() {
		CalQuaternion r = logic_delta_rotation;
		logic_delta_rotation.clear();
		return r;
	}
