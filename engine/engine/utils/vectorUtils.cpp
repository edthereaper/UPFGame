...
void straighten(Transform* t, const XMVECTOR& up)
{
    XMVECTOR left = XMVector3Cross(t->getUp(), up);
    if (left == zero_v) {
        left = xAxis_v;
    }
    float angle = XMVectorGetX(XMVector3AngleBetweenVectors(t->getUp(), up));
    t->applyRotation(XMQuaternionRotationAxis(left, angle));
}
